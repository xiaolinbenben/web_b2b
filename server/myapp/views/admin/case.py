...
def list_api(request):
    ...
    queryset = Case.objects.filter(title__icontains=keyword).order_by('-create_time')
...
def update(request):
    ...
    serializer = CaseSerializer(instance, data=request.data, partial=True)
    if serializer.is_valid():
        serializer.save()
...
def delete(request):
    ...
    Case.objects.filter(id__in=[item for item in ids.split(',') if item]).delete()
