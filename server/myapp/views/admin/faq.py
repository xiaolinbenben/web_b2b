...
def list_api(request):
    ...
    queryset = Faq.objects.filter(question__icontains=keyword).order_by('-create_time')
...
def update(request):
    ...
    try:
        instance = Faq.objects.get(pk=pk)
...
def delete(request):
    ...
    Faq.objects.filter(id__in=[item for item in ids.split(',') if item]).delete()
